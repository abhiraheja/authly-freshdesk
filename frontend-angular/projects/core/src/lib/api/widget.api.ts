...
export class WidgetApi {
   ...
   * The server sends `conversation` carrying a `{ conversationId }` and nothing
   * else, so the panel re-fetches through the endpoints that already apply the
   * trust rule and strip internal notes — a socket carrying message bodies would
   * be a second place invariant 5 has to hold.
   *
   ...
   * set a request header on `new WebSocket()`. A connection presenting neither
   * simply joins no group.
   *
