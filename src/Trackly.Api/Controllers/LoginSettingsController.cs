...
public class LoginSettingsController(TracklyDbContext db) : ControllerBase
{
    ...
    /// `Active` is only set once a real SSO login has completed, so this is proof too.
    private Task<bool> SsoActiveAsync(Guid workspaceId, CancellationToken ct)
        => db.SsoConnections.AnyAsync(c => c.WorkspaceId == workspaceId && c.Status == SsoStatus.Active, ct);
}
